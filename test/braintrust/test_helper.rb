...
class Braintrust::Test::SingletonClient < Braintrust::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: Braintrust::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class Braintrust::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @braintrust = Braintrust::Test::SingletonClient.instance
  end
