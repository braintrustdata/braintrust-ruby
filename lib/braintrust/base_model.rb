...
module Braintrust
  ...
  module Converter
    ...
    class << self
      ...
      def type_info(spec)
        ...
        in Braintrust::Converter | Class | Symbol
          -> { spec }
      ...
      def dump(target, value)
        target.is_a?(Braintrust::Converter) ? target.dump(value) : Braintrust::Unknown.dump(value)
      end
    end
  end
  ...
  module Union
    ...
    private def variant(key, spec = nil)
      variant_info =
        case key
        in Symbol
          [key, Braintrust::Converter.type_info(spec)]
        in Proc | Braintrust::Converter | Class | Hash
          [nil, Braintrust::Converter.type_info(key)]
        end
    ...
    private def resolve_variant(value)
      ...
      in [Symbol, Hash]
        key = value.fetch(@discriminator) do
          value.fetch(@discriminator.to_s, Braintrust::Util::OMIT)
        end

        return nil if key == Braintrust::Util::OMIT
    ...
    # @return [Boolean]
    def ==(other)
      other.is_a?(Module) && other.singleton_class <= Braintrust::Union && other.derefed_variants == derefed_variants
    end
    ...
    def coerce(value, state:)
      if (target = resolve_variant(value))
        return Braintrust::Converter.coerce(target, value, state: state)
      end
      ...
      known_variants.each do |_, variant_fn|
        ...
        coerced = Braintrust::Converter.coerce(target, value, state: state)
    ...
    def dump(value)
      if (target = resolve_variant(value))
        return Braintrust::Converter.dump(target, value)
      end

      known_variants.each do
        target = _2.call
        return Braintrust::Converter.dump(target, value) if target === value
      end
  ...
  class ArrayOf
    ...
    #
    # @return [Boolean]
    def ==(other) = other.is_a?(Braintrust::ArrayOf) && other.nilable? == nilable? && other.item_type == item_type
      ...
      value
        .map do |item|
          ...
          in [true, nil]
            ...
            Braintrust::Converter.coerce(target, item, state: state)
          end
        end
    end
    ...
    def dump(value)
      target = item_type
      value.is_a?(Array) ? value.map { Braintrust::Converter.dump(target, _1) } : super
    end
  ...
  class HashOf
    ...
    #
    # @return [Boolean]
    def ==(other) = other.is_a?(Braintrust::HashOf) && other.nilable? == nilable? && other.item_type == item_type
        ...
        .to_h do |key, val|
          ...
          v =
            ...
            in [true, nil]
              ...
              Braintrust::Converter.coerce(target, val, state: state)
            end
    ...
    def dump(value)
      target = item_type
      value.is_a?(Hash) ? value.transform_values { Braintrust::Converter.dump(target, _1) } : super
    end
  ...
  class BaseModel
    ...
    class << self
      ...
      private def add_field(name_sym, required:, type_info:, spec:)
        type_fn, info =
          case type_info
          in Proc | Braintrust::Converter | Class
            [Braintrust::Converter.type_info({**spec, union: type_info}), spec]
          in Hash
            [Braintrust::Converter.type_info(type_info), type_info]
          end

        setter = "#{name_sym}="
        api_name = info.fetch(:api_name, name_sym)
        nilable = info[:nil?]
        const = required && !nilable ? info.fetch(:const, Braintrust::Util::OMIT) : Braintrust::Util::OMIT
        ...
        define_method(name_sym) do
          target = type_fn.call
          value = @data.fetch(name_sym) { const == Braintrust::Util::OMIT ? nil : const }
          state = {strictness: :strong, exactness: {yes: 0, no: 0, maybe: 0}, branched: 0}
          if (nilable || !required) && value.nil?
            nil
          else
            Braintrust::Converter.coerce(
              target,
              value,
              state: state
            )
    ...
    class << self
      ...
      def coerce(value, state:)
        ...
        unless (val = Braintrust::Util.coerce_hash(value)).is_a?(Hash)
          exactness[:no] += 1
          return value
        end
        exactness[:yes] += 1
        ...
        fields.each do |name, field|
          ...
          converted =
            if item.nil? && (nilable || !required)
              ...
              coerced = Braintrust::Converter.coerce(target, item, state: state)
              case target
              in Braintrust::Converter | Symbol
                coerced
              else
                item
              end
      ...
      def dump(value)
        ...
        coerced.each do |key, val|
          ...
          in nil
            ...
            in :coerce
              ...
              target = type_fn.call
              acc.store(api_name, Braintrust::Converter.dump(target, val))
            end
          end
        end
        ...
        known_fields.each_value do |field|
          ...
          next if mode == :coerce || acc.key?(api_name) || const == Braintrust::Util::OMIT
    ...
    def inspect
      ...
      rescue Braintrust::ConversionError
        "#{_1}=#{@data.fetch(_1)}"
      end
